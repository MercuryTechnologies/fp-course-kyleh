...
instance Functor k => Functor (StateT s k) where
    (<$>) :: forall a b. (a -> b) -> StateT s k a -> StateT s k b
    (<$>) f prev = StateT next
        where
            next :: s -> k (b, s)
            next s = mapF <$> (runStateT prev s)
                where
                    mapF :: (a, s) -> (b, s)
                    mapF (a, s') = (f a, s')
...
instance Monad k => Applicative (StateT s k) where
    ...
    (<*>) (StateT context) (StateT rhs) = StateT next
        where
            next s = do
                (f, s') <- context s
                (a, s'') <- rhs s'
...
state' f = StateT (MakeExactlyOne . f)
...
runState' (StateT ef) x = res
    where
        (MakeExactlyOne res) = ef x
...
distinctF xs = evalT (filtering mp xs) S.empty
    where
        mp x = do
            seen <- getT
            if x > 100
              then StateT $ const Empty
              else if S.member x seen
                then pure False
                else do
                    putT $ S.insert x seen
                    pure True
...
instance Functor k => Functor (OptionalT k) where
    ...
    (<$>) f ot = OptionalT $ af <$> runOptionalT ot where
        af opt = f <$> opt
...
instance Monad k => Applicative (OptionalT k) where
    ...
    pure x = OptionalT { runOptionalT = pure (Full x) }

    (<*>) :: OptionalT k (a -> b) -> OptionalT k a -> OptionalT k b
    (<*>) OptionalT { runOptionalT = runContext } OptionalT { runOptionalT = runRhs } = OptionalT $ do
        optionalF <- runContext
        case optionalF of
            Empty -> pure Empty
            Full f -> do
                optionalX <- runRhs
...
instance Monad k => Monad (OptionalT k) where
    ...
    (=<<) f OptionalT { runOptionalT = runRhs } = OptionalT $ do
        optionalA <- runRhs
        case optionalA of
...
instance Functor (Logger l) where
    ...
    (<$>) f (Logger list x) = Logger list (f x)
...
instance Applicative (Logger l) where
    ...
    (<*>) (Logger soFar f) (Logger rhs a) = Logger (soFar ++ rhs) (f a)
...
instance Monad (Logger l) where
    ...
    (=<<) f (Logger soFar a) = Logger (soFar ++ nextLogs) b
        where
        Logger nextLogs b = f a
...
-}
log1 :: l -> a -> Logger l a
log1 message x = Logger (listh [message]) x
...
distinctG xs = runOptionalT $ evalT (filtering mp xs) S.empty
    where
